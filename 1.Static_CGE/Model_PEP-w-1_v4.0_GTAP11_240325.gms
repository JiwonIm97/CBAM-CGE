...
*SOLVE PEPW1 USING nlp MAXIMIZING OBJ;
 ...
 valEE_agr_agg(p_coal,j6,z,sim) = EEI(p_coal,j6,z)*DE.L('02_COAL',j6,z);
 valEE_agr_agg(p_oil,j6,z,sim) = EEI(p_oil,j6,z)*DE.L('03_OIL',j6,z);
 valEE_agr_agg(p_gas,j6,z,sim) = EEI(p_gas,j6,z)*DE.L('04_GAS',j6,z);
 valEE_agr_agg(p_oilproduct,j6,z,sim) = EEI(p_oilproduct,j6,z)*DE.L('10_PETROLCOAL',j6,z);
 valEE_agr_agg(p_elecheat,j6,z,sim) = EEI(p_elecheat,j6,z)*DE.L('18_ELEC',j6,z);

 valEE_ene_agg(p_coal,j7,z,sim) = EEI(p_coal,j7,z)*DE.L('02_COAL',j7,z);
 valEE_ene_agg(p_oil,j7,z,sim) = EEI(p_oil,j7,z)*DE.L('03_OIL',j7,z);
 valEE_ene_agg(p_gas,j7,z,sim) = EEI(p_gas,j7,z)*DE.L('04_GAS',j7,z);
 valEE_ene_agg(p_oilproduct,j7,z,sim) = EEI(p_oilproduct,j7,z)*DE.L('10_PETROLCOAL',j7,z);
 valEE_ene_agg(p_elecheat,j7,z,sim) = EEI(p_elecheat,j7,z)*DE.L('18_ELEC',j7,z);

 valEE_manu_agg(p_coal,j8,z,sim) = EEI(p_coal,j8,z)*DE.L('02_COAL',j8,z);
 valEE_manu_agg(p_oil,j8,z,sim) = EEI(p_oil,j8,z)*DE.L('03_OIL',j8,z);
 valEE_manu_agg(p_gas,j8,z,sim) = EEI(p_gas,j8,z)*DE.L('04_GAS',j8,z);
 valEE_manu_agg(p_oilproduct,j8,z,sim) = EEI(p_oilproduct,j8,z)*DE.L('10_PETROLCOAL',j8,z);
 valEE_manu_agg(p_elecheat,j8,z,sim) = EEI(p_elecheat,j8,z)*DE.L('18_ELEC',j8,z);
 
 valEE_trans_agg(p_coal,j9,z,sim) = EEI(p_coal,j9,z)*DE.L('02_COAL',j9,z);
 valEE_trans_agg(p_oil,j9,z,sim) = EEI(p_oil,j9,z)*DE.L('03_OIL',j9,z);
